...
inline int Lidar_gain::getOccupancy(const Eigen::Vector3i& id) {
    ...
    // make sure idex is in the range of data
    if (idex < 0 || idex >= OccGrid.data.size()) return OCCUPIED;
...
int Lidar_gain::Get_Multi_Raycast(vector<Eigen::Vector3d> ray_start, vector<Eigen::Vector3d> ray_end){
    ...
    if (!raycaster) {
        std::cerr << "Error: raycaster is not initialized." << std::endl;
        return -1;
    }

    int count = 0, d_count = 0;
    while (!ray_start.empty() && !ray_end.empty())
    {
        Eigen::Vector3d ray0 = ray_start.back();
        Eigen::Vector3d ray1 = ray_end.back();
        ray_start.pop_back();
        ray_end.pop_back();
...
int Lidar_gain::Get_Multi_Raycast(vector<Eigen::Vector3d> ray_start, vector<Eigen::Vector3d> ray_end, vector<Eigen::Vector3d>& ray_visual_){
    ...
    // Check if ray_start and ray_end is not NULL
    if (ray_start.empty() || ray_end.empty()) {
        std::cerr << "Error: ray_start or ray_end is empty." << std::endl;
    }

    // Check if ray_visual_ is not NULL
    if (ray_visual_.empty()){
        std::cerr << "Error: ray_visual_ is empty." << std::endl;
    }
    ...
    while (!ray_start.empty() && !ray_end.empty())
    {
        ...
        while (raycaster->next_Pos_idx(pos_, idx)){
            //Check point 7.1
            // std::cout << "***** Check Point 7.1 *****" << std::endl;
            if(getOccupancy(idx)==OCCUPIED || getOccupancy(idx)==-1){
                break;
            }
            d_count++;
            //Check point 7.2
            // std::cout << "***** Check Point 7.2 *****" << std::endl;
            if(d_count % 8 == 7)
                ray_visual_.push_back(pos_);
            //Check point 7.3
            // std::cout << "***** Check Point 7.3 *****" << std::endl;
        }
        //Check point 8
